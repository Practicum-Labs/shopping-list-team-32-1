...
private fun NavGraphBuilder.mainRoute(
    ...
    composable(
        ...
        MainRoute(
            ...
            onListClick = { listId ->
                navController.navigate("$PRODUCTS_ROUTE/$listId")
            },
        )
    }
}
...
private fun NavGraphBuilder.productsRoute(navController: NavHostController) {
    composable(
        route = "$PRODUCTS_ROUTE/{$PRODUCTS_LIST_ID_ARG}",
        arguments = listOf(
            navArgument(PRODUCTS_LIST_ID_ARG) { type = NavType.LongType },
        ),
    ) { backStackEntry ->
        val listId = backStackEntry.arguments?.getLong(PRODUCTS_LIST_ID_ARG) ?: return@composable
...
private const val PRODUCTS_LIST_ID_ARG = "listId"
